...
import { Article, CheckCircle, PlayArrow, Replay, Search, EmojiEvents, GpsFixed, Speed, TrendingUp } from '@mui/icons-material';
        ...
        <Box>
            ...
            <Box sx={{
                ...
                <Stack direction="row" alignItems="center" spacing={2} sx={{ position: 'relative', zIndex: 1 }}>
                    ...
                    <Box>
                        <Typography variant="h4" fontWeight={900} color="white">Letter Writing Practice</Typography>
                        <Typography sx={{ color: 'rgba(255,255,255,0.75)', mt: 0.5 }}>
                            Practice formal and business letter writing in a Word-like environment
                        </Typography>
                    </Box>
                </Stack>
            </Box>
            ...
            {!loading && templates.length > 0 && (
                <Grid container spacing={2.5} sx={{ mb: 4 }}>
                    {[
                        ...
                        { icon: <GpsFixed sx={{ fontSize: 28 }} />, label: 'Best Accuracy', value: attempted > 0 ? `${Math.round(bestAcc)}%` : '—', color: '#16a34a', bg: '#f0fdf4', sub: 'across all letters' },
                        { icon: <Speed sx={{ fontSize: 28 }} />, label: 'Best WPM', value: bestWpm > 0 ? String(bestWpm) : '—', color: '#2563eb', bg: '#eff6ff', sub: 'words per minute' },
                        { icon: <TrendingUp sx={{ fontSize: 28 }} />, label: 'Overall Progress', value: `${progress}%`, color: '#7c3aed', bg: '#f5f3ff', sub: `${templates.length - attempted} remaining` },
                    ].map(s => (
                    ...
                    <Grid item xs={12}>
                        <Paper elevation={0} sx={{ borderRadius: 3, border: '1px solid #e2e8f0', px: 3, py: 2 }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
                                ...
                                <Typography variant="body2" fontWeight={800} color="primary.main">{progress}%</Typography>
                            </Stack>
                            <LinearProgress variant="determinate" value={progress} sx={{ height: 10, borderRadius: 5, bgcolor: '#e2e8f0', '& .MuiLinearProgress-bar': { borderRadius: 5 } }} />
            ...
            <Paper elevation={0} sx={{ borderRadius: 3, border: '1px solid #e2e8f0', p: 2.5, mb: 3 }}>
                <TextField fullWidth size="small" placeholder="Search letters by name…" value={search}
            ...
            {/* ── List ── */}
            {loading
                ? <Stack spacing={2}>{[...Array(4)].map((_, i) => <Skeleton key={i} height={100} sx={{ borderRadius: 3, transform: 'none' }} />)}</Stack>
                : templates.length === 0
                    ? <Paper elevation={0} sx={{ p: 8, textAlign: 'center', borderRadius: 4, border: '2px dashed #e2e8f0' }}>
                        <Article sx={{ fontSize: 64, color: '#94a3b8', mb: 2 }} />
                        <Typography variant="h6" fontWeight={700} color="text.secondary">No letter templates available</Typography>
                        <Typography variant="body2" color="text.disabled">Your instructor hasn't added any letters yet.</Typography>
                    </Paper>
                    : filtered.length === 0
                            ...
                            {filtered.map(t => {
                                ...
                                return (
                                    <Paper key={t.id} elevation={0} sx={{
                                        borderRadius: 3, border: '1px solid', borderColor: t.attempted ? '#bfdbfe' : '#e2e8f0',
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'stretch' }}>
                                            <Box sx={{ width: 6, flexShrink: 0, bgcolor: t.attempted ? '#2563eb' : '#e2e8f0' }} />
                                            <Box sx={{ width: 64, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: t.attempted ? '#eff6ff' : '#f8fafc' }}>
                                                <Box sx={{ width: 40, height: 40, borderRadius: 2, bgcolor: t.attempted ? '#2563eb' : '#64748b', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                    {t.attempted ? <CheckCircle sx={{ color: 'white', fontSize: 22 }} /> : <Article sx={{ color: 'white', fontSize: 20 }} />}
                                                </Box>
                                            </Box>
                                            <Box sx={{ flex: 1, p: 2.5, minWidth: 0 }}>
                                                <Stack direction="row" alignItems="center" spacing={1.5} flexWrap="wrap" sx={{ mb: 0.5 }}>
                                                    ...
                                                    {t.attempted && <Chip size="small" label="Completed" icon={<CheckCircle sx={{ fontSize: '14px !important' }} />} color="primary" variant="outlined" sx={{ height: 22, fontSize: 11, fontWeight: 700 }} />}
                                                </Stack>
                                                {t.attempted
                                                    ? <Stack direction="row" spacing={3}>
                                                        ...
                                                        <Stack direction="row" alignItems="center" spacing={0.5}><Speed sx={{ fontSize: 14, color: '#2563eb' }} /><Typography variant="caption" fontWeight={700} color="primary.dark">{t.best_wpm} WPM</Typography></Stack>
                                                    </Stack>
                                                    : <Typography variant="caption" color="text.disabled">Not attempted yet</Typography>}
                                            </Box>
                                            <Box sx={{ display: 'flex', alignItems: 'center', pr: 2.5 }}>
                                                <Link href={`/student/practice/letter/${t.id}`} style={{ textDecoration: 'none' }}>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, px: 2.5, py: 1.2, borderRadius: 2.5, fontWeight: 700, fontSize: 13, bgcolor: t.attempted ? '#2563eb' : '#1e3a8a', color: 'white', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'opacity 0.2s, transform 0.1s', '&:hover': { opacity: 0.88, transform: 'scale(1.03)' } }}>
