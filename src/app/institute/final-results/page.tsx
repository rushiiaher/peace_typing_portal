...
import {
    ...
    Checkbox, Tooltip, Switch, Avatar,
} from '@mui/material';
import {
...
export default function InstituteFinalResultsPage() {
    ...
    // Derive unique courses from batches
    const courses = useMemo(() => {
        const map = new Map<string, string>();
        batches.forEach(b => { if (b.course_id) map.set(b.course_id, b.course_name); });
        return [...map.entries()].map(([id, name]) => ({ id, name }));
    }, [batches]);

    // Batches filtered by the selected course
    const batchOptions = useMemo(
        () => selectedCourse ? batches.filter(b => b.course_id === selectedCourse) : batches,
    ...
    const fetchResults = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/institute/final-results?${params}`);
    ...
    }, [selectedCourse, selectedBatch, dateFrom, dateTo, search]);
    ...
    const toggleSelectAll = () => {
        ...
        if (allDisplaySelected) displayRows.forEach(r => next.delete(r.id));
    ...
    const toggleCertificate = async (row: ResultRow) => {
        ...
        try {
            const res = await fetch('/api/institute/final-results', {
                method: 'PATCH',
        ...
        <AdminLayout menuItems={instituteAdminMenuItems} title="Institute Admin Panel">
            ...
            <Box sx={{ mb: 3 }}>
                ...
                <Typography color="text.secondary">
                    Completed exam results for your institute&apos;s students. Select rows for targeted downloads.
                </Typography>
            </Box>
                ...
                <Grid container spacing={2} alignItems="flex-end">
                    <Grid item xs={12} sm={6} md={3}>
                        <FormControl fullWidth size="small">
                            ...
                            <Select value={selectedCourse} label="Course"
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                        <FormControl fullWidth size="small">
                            ...
                            <Select value={selectedBatch} label="Batch" onChange={e => setSelectedBatch(e.target.value)}>
                                ...
                                {batchOptions.map(b => <MenuItem key={b.id} value={b.id}>{b.batch_name}</MenuItem>)}
                            </Select>
                        </FormControl>
                    </Grid>
                    <Grid item xs={12} sm={6} md={3}>
                                ...
                                <Table size="small" stickyHeader>
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            {['Photo', 'Student', 'Enroll No', "Mother's Name", 'Course', 'Batch', 'Marks', '%', 'Grade', 'Result', 'Exam Date', 'Cert Given'].map(h => (
