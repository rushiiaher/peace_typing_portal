...
// Resolve the logged-in institute admin's institute_id from the session.
// NEVER trust a client-supplied institute id — scope is enforced here.
...
export async function GET(req: NextRequest) {
    try {
        ...
        // Restrict to batches that belong to THIS institute only
        let bq = admin.from('batches').select('id').eq('institute_id', instituteId);
        if (batch_id) bq = bq.eq('id', batch_id);
        const { data: batchData, error: batchErr } = await bq;
        if (batchErr) throw batchErr;

        const batchIdFilter = (batchData ?? []).map((b: any) => b.id);
        ...
        console.error('[institute/final-results GET]', err);
        return NextResponse.json({ error: err.message }, { status: 500 });
    }
}

// PATCH — toggle certificate_generated, but only for exams whose batch
// belongs to this institute (prevents cross-institute tampering).
export async function PATCH(req: NextRequest) {
    try {
        ...
        // Verify every requested exam belongs to a batch of this institute
        const { data: ownExams, error: verifyErr } = await admin
            .from('exams')
            .select('id, batches!inner ( institute_id )')
            .in('id', ids)
            .eq('batches.institute_id', instituteId);
        if (verifyErr) throw verifyErr;

        const allowedIds = (ownExams ?? []).map((e: any) => e.id);
        if (allowedIds.length === 0) {
            return NextResponse.json({ error: 'No matching exams for this institute.' }, { status: 403 });
        }
        ...
        return NextResponse.json({ message: 'Updated.', updated: allowedIds.length });
    } catch (err: any) {
        return NextResponse.json({ error: err.message }, { status: 500 });
    }
}
